...
\begin{example}
  \begin{itemize}
    ...
    \item $\cat{Ring}$ is the category where the objects are rings and the morphisms are homomorphisms of rings.
...
\begin{exercise}[Automorphism group]
  Let $\cat{C}$ be a category. Define the \textbf{automorphisms} on the category $\cat{C}$ as a map
  \begin{align*}
  ...
  \begin{enumerate}
    ...
    \]
    \item What is the automorphisms group of the objects in the category $\cat{Set}$ and $\cat{Vec}_k$?
    \item Let $X$ be a topological space and $\Pi\bra{X}$ be its fundamental groupoid. Show that
    \begin{enumerate}
